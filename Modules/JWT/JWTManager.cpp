...
void JWTManager::encodeJWT(char *payloadToJWT, char *jwtEncoded) {
    if (payloadToJWT == nullptr || jwtEncoded == nullptr) {
        uartUSB.write("ERROR: NULL pointer in encodeJWT\r\n", 34);
        return;
    }

    uartUSB.write("payloadToJWT:\r\n", strlen("payloadToJWT:\r\n"));
    uartUSB.write(payloadToJWT, strlen(payloadToJWT));
    uartUSB.write("\r\n", 3);

    if (this->jwt->allocateJWTMemory() == false) {
        uartUSB.write("No memory for JWT\r\n", strlen("No memory for JWT\r\n"));
        return;
    }

    uartUSB.write(LOG_MESSAGE_GENERATING_JWT, LOG_MESSAGE_GENERATING_JWT_LEN);

    if (this->jwt->encodeJWT(payloadToJWT) == false) {
        uartUSB.write("JWT encoding failed\r\n", strlen("JWT encoding failed\r\n"));
        return;
    }
    ...
    // CRÍTICO: Copiar ANTES de llamar a clear()
    // Y usar memcpy en lugar de strncpy porque el JWT puede contener bytes NULL
    if (this->jwt->out != nullptr && this->jwt->outputLength > 0) {
        memcpy(jwtEncoded, this->jwt->out, this->jwt->outputLength);
        jwtEncoded[this->jwt->outputLength] = '\0';
        
        // Debug: verificar que no hay NULL bytes en medio
        char debugMsg[100];
        snprintf(debugMsg, sizeof(debugMsg), 
                "JWT copied: %d bytes\r\n", this->jwt->outputLength);
        uartUSB.write(debugMsg, strlen(debugMsg));
        
        // Verificar integridad
        bool hasEarlyNull = false;
        for (size_t i = 0; i < this->jwt->outputLength; i++) {
            if (jwtEncoded[i] == '\0') {
                snprintf(debugMsg, sizeof(debugMsg), 
                        "WARNING: NULL byte at position %d\r\n", i);
                uartUSB.write(debugMsg, strlen(debugMsg));
                hasEarlyNull = true;
                break;
            }
        ...
        if (!hasEarlyNull) {
            uartUSB.write("JWT integrity OK\r\n", 18);
        }
    } else {
        uartUSB.write("ERROR: Invalid JWT output buffer\r\n", 34);
    }
...
bool JWTManager::decodeJWT(char *jwtToDecode, char *payloadRetrieved) {
    ...
    // Copiar ANTES de clear()
    memcpy(payloadRetrieved, this->jwt->payload, this->jwt->payloadLength);
...
void JWTManager::encodeJWT(char *payloadToJWT, char *jwtEncoded) {
    ...
    uartUSB.write ("payloadToJWT:\r\n" , strlen ("payloadToJWT:\r\n"));  // debug only
    uartUSB.write (payloadToJWT , strlen (payloadToJWT));  // debug only
    uartUSB.write ( "\r\n",  3 );  // debug only}
