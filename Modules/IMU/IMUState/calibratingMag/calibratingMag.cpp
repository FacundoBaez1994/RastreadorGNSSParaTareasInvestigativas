...
bool calibratingMag::calibrate (I2CInterfaceHandler * interface, 
NonBlockingDelay * refresh) {
    ...
    if (refresh->read()) {
        switch (state) {
            ...
            case 2:
                ...
                if (ii < sample_count){
                    read_mag(mag_temp, interface);  // Read the mag data
                ...
                if (b_verbose) {
                    uartUSB.write("mag x min/max:\n\r", 15);
                    char buffer[10];
                    sprintf(buffer, "%.2hd", mag_min[0]);
                    uartUSB.write(buffer, strlen(buffer));
                    uartUSB.write(", ", 2);
                    sprintf(buffer, "%.2hd", mag_max[0]);
                    uartUSB.write(buffer, strlen(buffer));
                    uartUSB.write("\n\r", 2);

                    uartUSB.write("mag y min/max:\n\r", 15);
                    sprintf(buffer, "%.2hd", mag_min[1]);
                    uartUSB.write(buffer, strlen(buffer));
                    uartUSB.write(", ", 2);
                    sprintf(buffer, "%.2hd", mag_max[1]);
                    uartUSB.write(buffer, strlen(buffer));
                    uartUSB.write("\n\r", 2);

                    uartUSB.write("mag z min/max:\n\r", 15);
                    sprintf(buffer, "%.2hd", mag_min[2]);
                    uartUSB.write(buffer, strlen(buffer));
                    uartUSB.write(", ", 2);
                    sprintf(buffer, "%.2hd", mag_max[2]);
                    uartUSB.write(buffer, strlen(buffer));
                    uartUSB.write("\n\r", 2);
                }
                state = 3;
