...
void InertialSensor::initMPU9250() {  
       // Initialize MPU9250 device
        ...
        writeByte(MPU9250_ADDRESS, GYRO_CONFIG, c | this->Mscale << 3); // Set full scale range for the gyro
        ...
        writeByte(MPU9250_ADDRESS, ACCEL_CONFIG, c | this->Ascale << 3); // Set full scale range for the accelerometer
        ...
        wait_us (100000); // wait for pass-through mode enabled
}
...
void InertialSensor::calibrateMPU9250( ) {  
    ...
    // el bias deberia de sacarse de todos los ejes si se supera un cierto valor?
...
void InertialSensor::readAccelData() {
    ...
    this->ax = (float) accelCount[0] * this->aRes;
    this->ay = (float) accelCount[1] * this->aRes;
    this->az = (float) accelCount[2] * this->aRes;
