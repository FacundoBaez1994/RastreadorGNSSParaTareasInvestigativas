...
LoRa::LoRa () {
    this->ssPin = new DigitalOut (LORA_DEFAULT_SS_PIN);
...
LoRa::~LoRa () {
    ...
    this->resetPin = nullptr;
    delete this->spiInterface;
    this->spiInterface = nullptr;
}

bool LoRa::begin () {
    char USBString [60];

    this->resetPin->write(HIGH); 
    wait_us (200000);
    this->resetPin->write(LOW); 
    wait_us (200000);
    this->resetPin->write(HIGH); 
    wait_us (50000);
    ...
    //////
    snprintf(USBString, sizeof(USBString),"LoRa Device version %X", version); 
    uartUSB.write (USBString , strlen (USBString));  // debug only
    uartUSB.write ( "\r\n",  3 );  // debug only   
    if (version != 0x12) {
        return false;
    }
...
uint8_t LoRa::readRegister(uint8_t address)
{
    ...
    return this->singleTransfer(address & 0x7f, 0x00); 
}
...
uint8_t LoRa::singleTransfer(uint8_t address, uint8_t value)
{
    ...
    this->ssPin->write(LOW); // low state = device selected

    //  SPI transfer
    spiInterface->write(address);      // send address
    response = spiInterface->write(value);  // send value
